"""
CASE 05: FRESH HMD POSE BEFORE RENDER
BASE: main_vr_scene_7.py

목적:
    머리를 돌릴 때 월드/Z-depth가 기울거나 휘어 보이는 현상이
    waitGetPoses()에서 얻은 pose가 실제 렌더 시점에는 이미 오래된(stale) pose라서
    SteamVR/ALVR reprojection과 어긋나는 문제인지 확인한다.

이번 케이스에서 바꾼 것:
    - waitGetPoses()는 프레임 동기화용으로 그대로 유지
    - 실제 카메라에 넣는 HMD pose만 렌더 직전에
      getDeviceToAbsoluteTrackingPose(..., 0.0, ...)로 한 번 더 읽는다
    - prediction은 사용하지 않는다. 즉 "fresh pose" 효과만 단독으로 본다.
...
판정:
    baseline보다 머리 움직임에 따른 월드 휨/기울기 또는 지연감이 줄어들면
    pose timing / reprojection mismatch가 원인 후보이다.
    차이가 거의 없으면 기하(projection / eye transform / pose transform) 쪽 가능성이 더 크다.
...
def get_fresh_hmd_pose(vr_system, fallback_pose):
    """
    CASE 05:
    렌더 직전에 HMD pose를 한 번 더 읽는다.
    prediction(미래 시점)은 넣지 않고 0.0초 pose만 사용한다.
    실패하거나 invalid이면 waitGetPoses() 결과로 fallback한다.
    """
    try:
        ...
        if not getattr(get_fresh_hmd_pose, "_warned", False):
            ...
            print(f"[CASE05] fresh HMD pose 호출 실패 -> waitGetPoses pose 사용: {e}")
...
def main():
    ...
    print("=" * 68)
    print("CASE 05: FRESH HMD POSE BEFORE RENDER")
    print("baseline 유지 + 카메라용 HMD pose만 렌더 직전에 다시 획득")
    print("prediction=0 ms (fresh pose 효과만 테스트)")
    print("=" * 68)
    ...
    try:
        while True:
            ...
            # CASE 05 핵심 변경:
            # waitGetPoses()는 동기화에 사용하고,
            # 실제 카메라용 pose는 렌더 직전에 다시 얻는다.
