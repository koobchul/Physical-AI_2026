...
목적:
    baseline은 HMD의 회전은 추적하지만, 첫 유효 HMD 위치를 저장한 뒤
    머리의 위치 이동(translation)은 고정한다.

    이번 테스트는 SteamVR/ALVR이 현재 HMD의 6DoF pose를 기준으로
    reprojection하는데 애플리케이션 카메라는 translation을 고정하고 있어서
    공간이 흔들리거나 Z-depth가 미끄러지는 현상이 생기는지 확인한다.

이번 케이스에서 바꾼 것:
    - EyeRig의 lock_translation을 해제
    - HMD의 X/Y/Z 실제 위치 이동을 카메라에 그대로 반영
    - 회전, projection, eye transform, render/submit은 baseline 그대로
...
판정:
    머리를 좌우/상하로 "이동"했을 때 baseline보다 월드가 공간에 더 잘 고정되면
    translation lock과 runtime reprojection의 불일치가 원인 후보이다.

    반대로 작은 머리 이동에도 화면이 과하게 흔들리거나 불안정해진다면,
    baseline의 translation lock이 오히려 필요한 안정화였다는 뜻이다.

주의:
    이 케이스는 반드시 고개 회전뿐 아니라 실제로 머리를 좌/우, 앞/뒤로
    5~15cm 정도 천천히 이동해 비교하는 것이 좋다.
...
def main():
    ...
    # CASE 07 핵심 변경:
    # baseline의 "시작 위치 고정 + 회전 추적" 대신
    # 실제 HMD translation까지 그대로 카메라에 반영한다.
    args.full_6dof = True
    ...
    print("=" * 68)
    print("CASE 07: FULL 6DOF HEAD TRANSLATION")
    print("baseline 유지 + HMD 위치 이동(X/Y/Z)까지 카메라에 그대로 반영")
    print("※ 내일 고개 회전뿐 아니라 머리를 좌우/앞뒤로 실제 이동해 비교하세요.")
    print("=" * 68)
