"""
CASE 03: SYMMETRIC HORIZONTAL FRUSTUM CENTER
BASE: main_vr_scene_7.py

목적:
    머리를 돌릴 때 공간이 휘거나 Z-depth가 기울어지는 느낌이
    SteamVR의 비대칭 horizontal projection center를
    MuJoCo mjvGLCamera에 그대로 넣는 과정에서 생기는지 확인한다.

이번 케이스에서 바꾼 것:
    - 좌/우 눈의 frustum width는 baseline 그대로 유지
    - frustum_center만 0.0으로 강제하여 수평 projection을 대칭화
    - top/bottom, eye-to-head transform, HMD pose는 baseline 유지
...
판정:
    baseline보다 머리 회전 시 공간의 좌우 휨/원근 왜곡이 줄어들면
    frustum_center 적용이 원인 후보이다.
    대신 양안 정렬이나 입체감이 어색해질 수 있으므로
    "왜곡 감소"와 "양안 불일치"를 따로 기록한다.
...
class EyeRig:
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # CASE 03 핵심 변경:
            # baseline의 비대칭 projection center를 제거하고
            # 좌우 대칭 horizontal frustum으로 강제한다.
...
def main():
    ...
    print("=" * 68)
    print("CASE 03: SYMMETRIC HORIZONTAL FRUSTUM CENTER")
    print("baseline 유지 + cam.frustum_center만 0으로 강제")
    print("=" * 68)
