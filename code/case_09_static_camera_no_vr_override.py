...
목적:
    지금 보이는 월드/Z-depth 휨이
    EyeRig.apply()에서 HMD pose + eye transform + projection을
    MuJoCo camera에 덮어쓰는 과정에서 생기는지 크게 분리해서 확인한다.

이번 케이스에서 바꾼 것:
    - robosuite/MuJoCo scene은 그대로 렌더
    - HMD pose는 SteamVR에서 계속 받고 waitGetPoses() 동기화도 유지
    - 단, EyeRig.apply()를 호출하지 않아 MuJoCo 카메라를 HMD pose로 덮어쓰지 않는다
    - 즉, baseline의 base_cam으로 고정 렌더한다

그대로 유지한 것:
    - robosuite Lift 환경 / XArm7
    - BASIC controller / env.step()
    - 오른손 teleoperation / gripper
    - 왼손 입력 및 physics loop
    - SteamVR/OpenVR Scene 앱
    - MuJoCo stereo side-by-side render target
    - OpenGL FBO/blit
    - OpenVR IVRCompositor::Submit
    - waitGetPoses() 프레임 동기화
    - 전체 main loop 구조

판정:
    이 케이스에서 baseline의 특이한 휨/기울기가 사라진다면,
    원인은 HMD -> MuJoCo camera 변환(EyeRig.apply) 내부일 가능성이 매우 높다.

    반대로 HMD camera override를 완전히 껐는데도 같은 형태의 왜곡이 남는다면,
    render target / projection / submit / compositor 경로를 더 의심해야 한다.

주의:
    이 파일은 원인 분리용 진단 케이스다.
    머리 움직임과 MuJoCo camera가 정상적으로 1:1 추종하지 않는 것이 의도된 동작이다.
    "VR 조작감"을 평가하지 말고, 화면 자체의 휨/세로늘어짐/기울기만 비교한다.
...
조작:
    [머리]            HMD는 추적되지만 MuJoCo camera override는 OFF
    [오른손] 그립 홀드 : 로봇 추종 ON (클러치)
    [오른손] 트리거    : 그리퍼 닫기
    [왼손]  스틱      : 작업 공간 이동 입력은 유지
    [왼손]  트리거/그립: 상하 이동 입력은 유지
    Ctrl+C           : 종료
"""
...
def main():
    ...
    # CASE 09 핵심 변경:
    # HMD pose를 MuJoCo stereo camera에 덮어쓰는 EyeRig.apply()를 비활성화한다.
    # robosuite / rendering / OpenVR submit은 그대로 유지한다.
    args.no_vr_cam = True
    ...
    print("=" * 68)
    print("CASE 09: STATIC CAMERA / NO VR CAMERA OVERRIDE")
    print("robosuite/렌더/submit 유지 + EyeRig.apply(HMD camera override)만 OFF")
    print("※ 머리 추종 품질이 아니라 왜곡 자체가 남는지만 비교하세요.")
    print("=" * 68)
