...
목적:
    좌/우 눈의 eye-to-head transform 안에 포함된 회전(cant)이
    머리 회전 시 월드/Z-depth 기울기나 양안 왜곡을 만드는지 확인한다.

이번 케이스에서 바꾼 것:
    - SteamVR에서 받아온 좌/우 eye-to-head transform의 위치(IPD offset)는 유지
    - eye-to-head transform의 3x3 회전부만 identity로 강제
    - 즉, 좌/우 눈 위치는 그대로 두고 "눈 자체의 추가 회전"만 제거
...
판정:
    baseline보다 머리 회전 시 공간의 기울기/양안 불일치가 줄어들면
    eye-to-head 회전 성분이 원인 후보이다.

    변화가 거의 없다면 Quest/ALVR eye transform의 회전 성분은
    현재 문제의 주원인이 아닐 가능성이 높다.

주의:
    이 케이스는 IPD(좌우 눈 간격)는 유지한다.
    따라서 CASE 04처럼 입체감을 완전히 제거하는 테스트와는 다르다.
...
class EyeRig:
    def __init__(self, vr_system, roll_test=0.0, lock_translation=True):
        ...
        # CASE 10 핵심 변경:
        # 각 눈의 위치 offset(IPD)은 유지하고 eye-to-head의 회전만 제거한다.
...
def main():
    ...
    print("=" * 68)
    print("CASE 10: EYE TRANSFORM = TRANSLATION ONLY")
    print("baseline 유지 + eye-to-head의 위치(IPD)는 유지, 회전만 identity")
    print("=" * 68)
