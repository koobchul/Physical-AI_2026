"""
CASE 08: PREDICTED HMD POSE (+30 ms)
BASE: main_vr_scene_7.py

목적:
    머리를 빠르게 돌릴 때 월드/Z-depth가 뒤늦게 따라오거나 휘어 보이는 현상이
    motion-to-photon 지연과 pose timing mismatch 때문인지 확인한다.

이번 케이스에서 바꾼 것:
    - waitGetPoses()는 프레임 동기화용으로 그대로 유지
    - 실제 카메라용 HMD pose는 렌더 직전에 다시 획득
    - getDeviceToAbsoluteTrackingPose()의 predictedSecondsToPhotonsFromNow에
      +0.030초(30 ms)를 사용
    - 즉, CASE 05의 fresh pose(0 ms)와 비교하여 "미래 pose 예측" 효과만 본다.
...
그대로 유지한 것:
    ...
    - 왼손 작업공간 이동
    - HMD 절대 pose를 카메라에 적용하는 방식
    - eye-to-head transform / projection frustum
    - SteamVR native render-target 해상도/종횡비
    - MuJoCo stereo side-by-side 렌더
    - OpenGL FBO/blit
    - OpenVR IVRCompositor::Submit
    - physics 20Hz / VR render loop 구조

판정:
    CASE 05(0 ms)보다 머리를 돌릴 때 월드가 더 단단히 고정되고
    지연/휘어짐이 줄어들면 prediction이 유효한 후보이다.

    반대로 화면이 앞서가거나 과하게 흔들리면 30 ms가 너무 큰 값일 수 있다.
    그 경우 다음에는 15 ms 같은 중간값을 비교할 수 있다.
...
POSE_PREDICT_S = 0.030


def get_predicted_hmd_pose(vr_system, fallback_pose):
    """
    CASE 08:
    렌더 직전에 +30 ms 미래 HMD pose를 요청한다.
    호출 실패/invalid 시 waitGetPoses() 결과로 fallback한다.
    """
    try:
        ...
        if not getattr(get_predicted_hmd_pose, "_warned", False):
            ...
            print(f"[CASE08] predicted HMD pose 호출 실패 -> waitGetPoses pose 사용: {e}")
...
def main():
    ...
    print("=" * 68)
    print("CASE 08: PREDICTED HMD POSE (+30 ms)")
    print("baseline 유지 + 카메라용 HMD pose를 렌더 직전에 30 ms 미래값으로 재획득")
    print("CASE 05(0 ms fresh pose)와 직접 비교하세요.")
    print("=" * 68)
    ...
    try:
        while True:
            ...
            # CASE 08 핵심 변경:
            # 실제 카메라에는 렌더 직전에 얻은 +30 ms predicted pose를 사용한다.
