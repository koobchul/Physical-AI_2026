...
목적:
    머리를 돌릴 때 공간/Z-depth가 휘어 보이거나 화면이 세로로 길게 느껴지는 현상이
    SteamVR projection raw의 horizontal width를 MuJoCo frustum_width에 직접 넣으면서,
    실제 렌더 viewport의 종횡비와 불일치해서 생기는지 확인한다.

이번 케이스에서 바꾼 것:
    - 렌더 해상도는 baseline과 동일하게 SteamVR native render-target 비율을 사용
    - frustum_bottom / frustum_top / frustum_center는 baseline 그대로 유지
    - frustum_width만 실제 렌더 viewport 종횡비(EW/EH)에 맞춰 계산
      width = (top - bottom) * NEAR * render_aspect
    - 즉, horizontal FOV 폭만 viewport 비율과 일치시키는 테스트
...
그대로 유지한 것:
    ...
    - SteamVR native render-target 해상도
...
판정:
    baseline보다 화면 비율과 원근이 자연스럽고 머리 회전 시 휨이 줄어들면
    raw horizontal frustum width와 실제 render aspect의 불일치가 원인 후보이다.
    좌우 시야가 너무 좁거나 넓어지면 그 변화도 함께 기록한다.
...
class EyeRig:
    def __init__(self, vr_system, roll_test=0.0, lock_translation=True):
        ...
        # CASE 06: 실제 eye viewport의 EW/EH를 main()에서 주입한다.
        self.render_aspect = None
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            try:
                # CASE 06 핵심 변경:
                # baseline: (right - left) * NEAR
                # 여기서는 vertical frustum height와 실제 viewport aspect로
                # horizontal width를 다시 만들어 projection과 렌더 비율을 맞춘다.
                if self.render_aspect is not None:
                    cam.frustum_width = (top - bottom) * NEAR * self.render_aspect
                else:
...
def main():
    ...
    eyes.render_aspect = EW / EH
    print(f"[CASE06] frustum_width 계산에 실제 렌더 종횡비 {eyes.render_aspect:.4f} 사용")
    ...
    print("=" * 68)
    print("CASE 06: FRUSTUM WIDTH FROM RENDER ASPECT")
    print("baseline 유지 + frustum_width만 실제 eye viewport 종횡비로 재계산")
    print("=" * 68)
