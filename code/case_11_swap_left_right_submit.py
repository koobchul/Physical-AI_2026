"""
CASE 11: SWAP LEFT / RIGHT EYE SUBMIT
BASE: main_vr_scene_7.py

목적:
    baseline에서 생성한 좌/우 눈 이미지가 실제 HMD 좌/우 눈과
    반대로 매핑되고 있는 가능성을 빠르게 확인한다.

이번 케이스에서 바꾼 것:
    - MuJoCo는 baseline과 동일하게 좌/우 stereo 이미지를 렌더
    - OpenVR Submit 단계에서만 좌/우 texture bounds를 서로 바꿔 제출
      Left eye  <- baseline의 right-half image
      Right eye <- baseline의 left-half image
...
판정:
    정상적인 시스템이라면 이 케이스는 baseline보다 깊이감이 훨씬 이상해지거나
    눈이 불편해지는 것이 예상된다.

    그런데 오히려 이 케이스가 baseline보다 자연스럽고 Z-depth가 정상적으로 보인다면,
    현재 SBS 좌/우 이미지 또는 Submit eye mapping이 뒤집혀 있을 가능성이 매우 높다.

주의:
    불편감이 강하면 오래 보지 말고 바로 종료한다.
    이 케이스는 "더 좋아지는지/더 나빠지는지"만 빠르게 확인하는 진단용이다.
...
class StereoTarget:
    ...
    def submit(self, comp, same_both=False):
        if same_both:
            ...
            # CASE 11 핵심 변경:
            # baseline의 좌/우 제출을 서로 바꿔 eye mapping을 진단한다.
            comp.submit(openvr.Eye_Left, self.texture, self.bounds_r)
            comp.submit(openvr.Eye_Right, self.texture, self.bounds_l)
...
def main():
    ...
    print("=" * 68)
    print("CASE 11: SWAP LEFT / RIGHT EYE SUBMIT")
    print("baseline 유지 + OpenVR Submit 단계에서 좌/우 eye image만 서로 교환")
    print("※ 눈이 많이 불편하면 바로 종료하세요.")
    print("=" * 68)
