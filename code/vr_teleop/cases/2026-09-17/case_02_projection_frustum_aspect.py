...
목적:
    baseline에서 화면이 세로로 길고 좁아 보이거나,
    머리를 돌릴 때 원근/Z-depth가 비정상적으로 느껴지는 현상이
    "SteamVR 권장 render-target 종횡비"와 "실제 eye projection frustum 종횡비"
    사이의 불일치 때문인지 확인한다.

이번 케이스에서 바꾼 것:
    - 눈당 렌더 해상도의 가로/세로 비율을 SteamVR native render-target 비율이 아니라
      getProjectionRaw()에서 얻은 실제 projection frustum 비율로 강제한다.
    - 그 외 HMD pose, eye transform, stereo camera, physics, controller는 baseline 유지.
...
판정:
    baseline보다 화면 비율이 자연스럽고 머리 회전 시 공간의 휨이 줄어들면
    render-target aspect와 projection frustum aspect 불일치가 원인 후보이다.
    변화가 없으면 다음 케이스에서 frustum 자체(비대칭/center/width)를 분리해서 본다.
...
def main():
    ...
    # CASE 02 핵심 변경:
    # baseline의 SteamVR native render-target 비율 대신
    # 실제 eye projection frustum 비율을 렌더 해상도에 사용한다.
    ...
    print("=" * 68)
    print("CASE 02: PROJECTION FRUSTUM ASPECT")
    print("baseline 유지 + 렌더 종횡비만 eye projection frustum 기준으로 변경")
    print("=" * 68)
