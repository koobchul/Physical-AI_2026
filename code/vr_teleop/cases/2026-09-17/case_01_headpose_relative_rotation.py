"""
CASE 01: STARTUP-RELATIVE HEAD ROTATION
BASE: main_vr_scene_7.py

목적:
    머리를 돌릴 때 월드/Z-depth가 같이 기울거나 휘어 보이는 현상이
    HMD의 "절대 회전"을 MuJoCo 카메라에 직접 넣어서 생기는지 확인한다.

이번 케이스에서 바꾼 것:
    - 첫 유효 HMD 회전을 기준 자세 R_ref로 저장
    - 이후 카메라 회전은 R_ref.T @ R_current 의 상대 회전만 사용
    - HMD 위치는 baseline과 동일하게 시작 위치에 고정
...
판정:
    baseline(main_vr_scene_7.py)보다 머리 회전 시 공간이 덜 기울거나
    Z-depth가 안정되면 "절대 HMD 회전"이 원인 후보이다.
    차이가 없으면 projection/frustum/eye transform 쪽을 다음 케이스에서 본다.
...
class EyeRig:
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        if self._head_rot_ref is None:
            self._head_rot_ref = hmd[:3, :3].copy()
            mode = "상대 회전 + 시작 위치 고정" if self.lock_translation else "상대 회전 + full 6DoF"
            print(f"[CASE01] HMD 시작 회전을 정면 기준으로 저장 / {mode}")
            print(f"[CASE01] HMD 기준 위치 저장: {self._head_pos_ref}")

        # 핵심 변경:
        # tracking-space의 절대 회전을 그대로 쓰지 않고,
        # 실행 시작 시 HMD 방향을 identity로 만드는 상대 회전만 사용한다.
        hmd[:3, :3] = self._head_rot_ref.T @ hmd[:3, :3]

        if self.lock_translation:
            # baseline과 동일하게 위치는 시작 위치에 고정한다.
            # 이번 케이스에서는 translation 동작을 바꾸지 않는다.
...
def main():
    ...
    drain_gl_errors()
    print("=" * 68)
    print("CASE 01: STARTUP-RELATIVE HEAD ROTATION")
    print("baseline 유지 + HMD 회전만 시작 자세 기준 relative rotation으로 변경")
    print("=" * 68)
