...
목적:
    SteamVR getProjectionRaw()의 top/bottom 부호 규약을 MuJoCo frustum에 옮길 때
    세로 방향 부호가 뒤집혀 적용되고 있는지 확인한다.

baseline에서는 SteamVR의 top/bottom 값을 min/max로 정렬한 뒤
cam.frustum_bottom / cam.frustum_top에 그대로 넣는다.

이번 케이스에서 바꾼 것:
    - horizontal frustum(center/width)은 baseline 그대로 유지
    - vertical frustum만 부호를 반전하여 적용
      CASE 15:
        frustum_bottom = -top * NEAR
        frustum_top    = -bottom * NEAR
    - HMD pose / eye transform / IPD / render target은 baseline 유지
...
그대로 유지한 것:
    ...
    - horizontal projection center / width
...
판정:
    baseline에서 특히 상하로 고개를 움직일 때 화면이 휘거나 공간이
    위/아래 방향으로 비틀리는 느낌이 있었다면 이 케이스와 비교한다.

    - CASE 15가 훨씬 자연스러움:
      SteamVR <-> MuJoCo vertical projection 부호 규약 불일치 가능성이 높다.
    - 훨씬 더 이상해짐:
      baseline의 vertical sign이 맞았을 가능성이 높다.
    - 거의 차이 없음:
      vertical frustum sign은 주원인이 아닐 가능성이 높다.

주의:
    이 케이스는 의도적으로 projection의 세로 부호를 뒤집는 진단용이다.
    화면이 불편하면 오래 보지 말고 바로 종료한다.
...
class EyeRig:
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            cam.frustum_far = FAR
            # CASE 15 핵심 변경:
            # SteamVR와 MuJoCo의 vertical sign convention이 반대인지 보기 위해
            # top/bottom의 부호만 뒤집는다. horizontal projection은 그대로 둔다.
            cam.frustum_bottom = -top * NEAR
            cam.frustum_top = -bottom * NEAR
...
def main():
    ...
    print("=" * 68)
    print("CASE 15: FLIP VERTICAL FRUSTUM SIGN")
    print("baseline 유지 + vertical frustum(top/bottom) 부호만 반전")
    print("※ 특히 고개를 위/아래로 움직일 때 baseline과 비교하세요.")
    print("=" * 68)
