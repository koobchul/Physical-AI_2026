"""
CASE 29: PITCH-ONLY + WORLD-UP LOCK
DATE: 2026-09-18
BASE: case_13_world_up_lock_no_roll.py

CASE 28(yaw-only)의 보완 실험.
남은 distortion이 pitch 자체에서 생기는지 분리하기 위한 진단 케이스.

배경:
    - CASE 09: HMD camera override를 끄면 distortion이 사라짐.
    - CASE 13: world-up + no-roll에서 안정성이 크게 좋아졌지만
      residual distortion은 남았음.
    - CASE 28: yaw만 남기고 pitch/roll을 제거하여 horizontal rotation을 분리함.
    - 따라서 이번에는 반대로 yaw를 고정하고 pitch만 남겨
      vertical rotation이 distortion을 유발하는지 확인한다.

목적:
    첫 유효 HMD pose의 horizontal heading(yaw)을 기준 방향으로 저장하고,
    이후에는 현재 HMD의 pitch angle만 camera forward에 반영한다.
...
이번 케이스에서 바꾼 것:
    - CASE 13 대비 yaw를 시작 시점 방향으로 고정.
    - 현재 HMD forward에서 pitch angle만 추출해서 적용.
    - position / IPD / projection / timing은 그대로 유지.
...
내일 확인할 것:
    1. 위/아래(pitch) 움직임에서 world bending이 생기는가?
    2. floor / table이 pitch에서 휘거나 늘어나는가?
    3. CASE 28(yaw-only)보다 distortion이 더 큰가?
    4. pitch-only 상태에서 vertically narrower 느낌이 더 강해지는가?
    5. yaw를 해도 virtual camera heading이 고정되는가?
       -> 이건 정상 동작이며 진단 목적.
    6. latency / buffering은 CASE 13과 비슷한가?

판정:
    - CASE 29에서 distortion이 크고 CASE 28은 상대적으로 안정:
        pitch 성분 또는 vertical projection coupling이 강한 원인 후보.
    - CASE 28에서만 크게 문제:
        yaw / horizontal mapping 쪽이 더 강한 후보.
    - 둘 다 문제:
        공통 camera-frame / projection 경로를 더 봐야 함.
    - 둘 다 안정:
        yaw+pitch 결합(coupling)에서만 문제가 생길 가능성이 큼.

주의:
    이 Case에서는 좌우로 고개를 돌려도 virtual camera yaw가 따라가지 않는다.
    먼저 정면에서 작은 pitch up/down만 천천히 테스트한다.
"""
...
class EyeRig:
    def __init__(self, vr_system, roll_test=0.0, lock_translation=True):
        ...
        # CASE 29: 첫 유효 pose의 horizontal heading(yaw)을 고정하기 위한 기준.
        self._pitch_only_heading_ref = None
        self.eye_to_head = [
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # CASE 29 핵심 변경:
            # 첫 유효 pose의 horizontal heading(yaw)은 고정하고,
            # 현재 forward에서 pitch angle만 추출해 camera에 적용한다.
            horizontal = forward_raw.copy()
            horizontal[2] = 0.0
            horizontal_norm = np.linalg.norm(horizontal)

            if self._pitch_only_heading_ref is None:
                if horizontal_norm < 1e-6:
                    # 극단적으로 위/아래를 보고 시작했다면 안전한 기본 heading 사용.
                    self._pitch_only_heading_ref = np.array(
                        [1.0, 0.0, 0.0], dtype=float
                    )
                else:
                    self._pitch_only_heading_ref = horizontal / horizontal_norm
                print(
                    "[CASE29] pitch-only 기준 horizontal heading 저장:",
                    self._pitch_only_heading_ref,
                )

            heading = self._pitch_only_heading_ref

            # unit forward의 z 성분으로 pitch를 추출.
            pitch = np.arcsin(np.clip(forward_raw[2], -1.0, 1.0))

            # yaw는 heading으로 고정하고 pitch만 실시간 반영.
            forward = (
                np.cos(pitch) * heading
                + np.sin(pitch) * np.array([0.0, 0.0, 1.0], dtype=float)
            )
            forward = forward / max(np.linalg.norm(forward), 1e-12)

            # CASE 13과 동일하게 roll 제거 / world-up 기준 up 재구성.
...
def main():
    ...
    # CASE 29는 pitch-only 진단이므로 외부 roll-test 주입은 비활성화한다.
    args.roll_test = 0.0
    ...
    print("=" * 72)
    print("CASE 29: PITCH-ONLY + WORLD-UP LOCK")
    print("BASE = CASE 13")
    print("변경 = yaw 시작값 고정 / pitch만 실시간 적용 / roll 제거")
    print("※ 정면에서 작은 pitch up/down부터 천천히 확인하세요.")
    print("=" * 72)
