"""
CASE 20: NO-ROLL + PREDICTED HMD POSE (15 ms)
DATE: 2026-09-18
BASE: case_13_world_up_lock_no_roll.py

09/17 결과를 바탕으로 한 2차 timing 진단 실험.

배경:
    - CASE 13: world-up 고정 + no-roll에서 체감 안정성이 크게 개선됨.
    - CASE 08: +30 ms predicted pose에서 latency는 일부 개선되었지만
      world-lock / floor movement 문제는 남았음.
    - CASE 19: CASE 13 + fresh 0 ms pose를 비교하기 위한 실험.

목적:
    CASE 13의 안정적인 camera frame(world-up/no-roll)을 유지한 채,
    렌더 직전 +15 ms predicted HMD pose를 사용한다.

    CASE 19(0 ms)와 직접 비교하여
    작은 prediction이 motion-to-photon latency를 줄이면서도
    overshoot / swimming / distortion을 악화시키지 않는지 확인한다.

이번 케이스에서 바꾼 것:
    - CASE 13 대비 HMD pose timing만 변경.
    - waitGetPoses()는 compositor 동기화용으로 그대로 유지.
    - 실제 MuJoCo camera에는 렌더 직전
      getDeviceToAbsoluteTrackingPose(..., +0.015 s, ...) pose를 사용.
    - invalid / exception 시 waitGetPoses pose로 fallback.

그대로 유지한 것:
    - CASE 13 world-up lock
    - CASE 13 camera roll 제거
    - HMD yaw / pitch
    - baseline translation lock
    - eye-to-head transform / IPD
    - SteamVR projection frustum
    - robosuite Lift / XArm7
    - BASIC controller
    - env.reset() / env.step()
    - MuJoCo stereo side-by-side
    - OpenGL FBO / GPU blit
    - OpenVR IVRCompositor::Submit
    - physics / input 구조

내일 확인할 것:
    1. CASE 19(0 ms)보다 latency가 줄어드는가?
    2. 빠른 yaw / pitch에서 world가 더 단단히 고정되는가?
    3. head stop 직후 overshoot가 생기지 않는가?
    4. floor / table이 미끄러지거나 따라오는 느낌이 줄어드는가?
    5. jitter가 증가하지 않는가?
    6. distortion 자체가 CASE 13/19보다 줄어드는가?

판정:
    - CASE 20 > CASE 19:
        약한 prediction이 timing 보정에 도움이 될 가능성이 큼.
    - CASE 20 ≈ CASE 19:
        prediction 15 ms의 이득이 작음.
    - CASE 20 < CASE 19:
        prediction이 reprojection / camera timing을 더 어긋나게 할 가능성.
    - latency만 개선되고 distortion 동일:
        timing과 geometry 문제는 서로 별개일 가능성이 높음.

주의:
    CASE 08의 +30 ms보다 작은 +15 ms를 쓰는 이유는
    prediction 과다로 인한 overshoot를 줄이면서 timing 이득만 확인하기 위해서다.
"""
...
class EyeRig:
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # CASE 20에서도 CASE 13 camera frame을 그대로 유지:
            # HMD가 정한 forward(yaw/pitch)는 그대로 쓰되,
            # up은 월드 +Z를 forward에 직교 투영해 camera roll을 제거한다.
            forward = forward / max(np.linalg.norm(forward), 1e-12)
            world_up = np.array([0.0, 0.0, 1.0], dtype=float)
            up = world_up - forward * np.dot(world_up, forward)
            up_norm = np.linalg.norm(up)

            # 정면이 거의 수직(+/-Z)일 때는 world_up 투영이 퇴화하므로
            # baseline의 eye Y축을 fallback으로 사용한다.
            if up_norm < 1e-6:
                up = axes[:, 1]
                up = up - forward * np.dot(up, forward)
                up_norm = np.linalg.norm(up)

            up = up / max(up_norm, 1e-12)
...
POSE_PREDICT_S = 0.015


def get_predicted_hmd_pose(vr_system, fallback_pose):
    """
    CASE 20:
    waitGetPoses()는 compositor 동기화용으로 유지하고,
    실제 camera 적용 직전에 +15 ms predicted HMD pose를 읽는다.
    invalid / exception 시 waitGetPoses() pose를 사용한다.
    """
    try:
        predicted_poses = vr_system.getDeviceToAbsoluteTrackingPose(
            openvr.TrackingUniverseStanding,
            POSE_PREDICT_S,
            openvr.k_unMaxTrackedDeviceCount,
        )
        predicted = predicted_poses[openvr.k_unTrackedDeviceIndex_Hmd]
        if predicted.bPoseIsValid:
            return predicted
    except Exception as e:
        if not getattr(get_predicted_hmd_pose, "_warned", False):
            get_predicted_hmd_pose._warned = True
            print(f"[CASE20] predicted HMD pose 실패 -> waitGetPoses pose fallback: {e}")
...
def main():
    ...
    # CASE 13은 roll 제거 자체를 검사하므로 외부 roll-test 주입은 비활성화한다.
    args.roll_test = 0.0
    ...
    print("=" * 72)
    print("CASE 20: NO-ROLL + PREDICTED HMD POSE (+15 ms)")
    print("BASE = CASE 13 world-up/no-roll")
    print("추가 변경 = 렌더 직전 +15 ms predicted HMD pose")
    print("CASE 19(0 ms)와 latency / overshoot / world-lock을 직접 비교하세요.")
    print("=" * 72)
    ...
    try:
        while True:
            ...
            # CASE 20 핵심 변경:
            # waitGetPoses()는 frame synchronization에 사용하고,
            # 실제 camera에는 렌더 직전 +15 ms predicted pose를 사용한다.
            hmd_pose = get_predicted_hmd_pose(vr_system, hmd_pose_wait)
