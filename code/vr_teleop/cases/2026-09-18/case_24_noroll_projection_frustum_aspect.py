...
09/17 결과에서 남은 두 문제를 분리해서 함께 확인하는 실험.

배경:
    - CASE 13: world-up 고정 + no-roll에서 motion comfort와 안정성이 크게 개선됨.
      그러나 화면이 vertically narrower하게 보이는 문제가 남았음.
    - CASE 16: render aspect를 SteamVR projection frustum aspect에 거의 정확히
      맞춰도 distortion 자체는 남았음.
      -> aspect mismatch는 distortion의 주원인은 아닐 가능성이 큼.
    - 하지만 CASE 13에서 남은 "세로로 좁아 보임"은 distortion과 별개의
      projection / render-aspect 문제일 수 있음.

목적:
    CASE 13의 world-up/no-roll camera orientation은 그대로 유지하면서,
    eye render resolution의 aspect를 SteamVR getProjectionRaw()의
    frustum aspect에서 계산하도록 강제한다.

    즉:
        motion 안정화 : CASE 13 설정 유지
        화면비/FOV    : projection frustum aspect 사용

    distortion과 화면비 문제를 분리해서,
    "motion은 안정적인데 화면만 좁아 보이는" 문제를 줄일 수 있는지 확인한다.

이번 케이스에서 바꾼 것:
    - CASE 13 대비 딱 하나:
      args.use_frustum_aspect = True
    - render resolution aspect를 SteamVR recommended target ratio 대신
      projection frustum의 horizontal/vertical span 비율로 계산한다.
...
그대로 유지한 것:
    ...
    - frustum center / top / bottom / width 계산 방식
...
내일 확인할 것:
    1. CASE 13의 vertically narrower 현상이 줄어드는가?
    2. 정사각형 / 원형 물체의 비율이 더 자연스러운가?
    3. horizontal / vertical FOV 균형이 좋아지는가?
    4. CASE 13의 motion comfort / no-roll 안정성은 유지되는가?
    5. head yaw/pitch 시 distortion이 증가하지 않는가?
    6. stereo alignment / eye strain이 악화되지 않는가?

판정:
    - 화면비만 좋아지고 distortion은 CASE 13과 비슷:
        aspect는 별도의 visual-shape 문제였다는 강한 증거.
    - 화면비와 distortion 모두 개선:
        projection aspect와 camera-frame 문제가 함께 기여했을 가능성.
    - CASE 13보다 더 나빠짐:
        frustum aspect 기반 resolution이 현재 compositor/render path와
        맞지 않을 가능성.
    - 차이 없음:
        vertical narrowing은 aspect가 아닌 다른 projection parameter 문제일 가능성.

주의:
    이 Case의 1차 목표는 distortion 해결이 아니라
    CASE 13에서 남았던 화면비 / FOV 문제를 분리하는 것이다.
"""
...
class EyeRig:
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # CASE 24에서도 CASE 13 orientation을 그대로 유지:
...
def main():
    ...
    # CASE 24 핵심 변경:
    # CASE 13의 no-roll/world-up은 유지하고,
    # eye render resolution aspect만 projection frustum에서 계산한다.
    args.use_frustum_aspect = True

    # CASE 13과 동일하게 외부 roll-test 주입은 비활성화한다.
    args.roll_test = 0.0
    ...
    print("=" * 72)
    print("CASE 24: NO-ROLL + PROJECTION FRUSTUM ASPECT")
    print("BASE = CASE 13 world-up/no-roll")
    print("추가 변경 = render resolution aspect를 projection frustum에서 계산")
    print("CASE 13과 vertical narrowing / FOV / distortion을 직접 비교하세요.")
    print("=" * 72)
