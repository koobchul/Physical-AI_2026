"""
CASE 23: EXPLICIT CAMERA BASIS + EYE TRANSLATION ONLY
DATE: 2026-09-18
BASE: case_22_explicit_camera_basis.py

09/17 결과와 CASE 22의 camera-frame 가설을 결합한 진단 실험.

배경:
    - CASE 10: eye-to-head rotation을 제거하고 translation(IPD)만 유지했을 때
      stereo separation / depth는 정상이고, no buffering / high resolution /
      preferred로 기록됨.
    - CASE 13: world-up + no-roll에서 체감 안정성이 크게 개선됨.
    - CASE 22: forward/right/up을 explicit orthonormal basis로 재구성해
      camera-frame coupling 여부를 검사하도록 설계됨.

목적:
    CASE 22의 explicit orthonormal camera basis는 그대로 유지하고,
    eye-to-head transform에서 rotation만 제거한다.

    즉:
        HMD forward(yaw/pitch)      : 사용
        camera roll                : 제거
        camera right/up basis      : explicit cross-product 방식
        eye translation / IPD      : 유지
        eye-to-head rotation(cant) : 제거

    CASE 22 대비 eye rotation만 제거했을 때
    residual distortion / eye alignment / motion comfort가 더 좋아지는지 확인한다.

이번 케이스에서 바꾼 것:
    - CASE 22 대비 딱 하나:
      각 eye_to_head transform의 3x3 rotation을 identity로 교체.
    - eye translation(IPD)은 그대로 유지.
...
내일 확인할 것:
    1. CASE 22보다 yaw/pitch 시 geometry distortion이 줄어드는가?
    2. floor / table / robot이 더 안정적으로 world-locked 되는가?
    3. stereo depth는 정상적으로 유지되는가?
    4. double vision / eye strain이 증가하지 않는가?
    5. CASE 10에서 보였던 horizontally narrower 현상이 다시 나타나는가?
    6. latency / buffering은 CASE 22와 비슷하게 유지되는가?

판정:
    - CASE 23 > CASE 22:
        eye-to-head rotation이 residual distortion / alignment에 기여했을 가능성 큼.
    - CASE 23 ≈ CASE 22:
        eye rotation은 핵심이 아니고 HMD camera frame 자체가 더 중요할 가능성.
    - CASE 23 < CASE 22:
        eye rotation이 현재 projection과 일관되게 필요한 요소일 가능성.

주의:
    이 파일은 진단용이다.
    stereo가 불편하거나 double vision이 증가하면 오래 보지 말고 즉시 종료한다.
"""
...
class EyeRig:
    def __init__(self, vr_system, roll_test=0.0, lock_translation=True):
        ...
        # CASE 23 핵심 변경:
        # CASE 22 camera basis는 그대로 두고,
        # eye-to-head translation(IPD)은 유지한 채 rotation(cant)만 제거한다.
        self._eye_rot_original = [
            self.eye_to_head[0][:3, :3].copy(),
            self.eye_to_head[1][:3, :3].copy(),
        ]
        for e in self.eye_to_head:
            e[:3, :3] = np.eye(3, dtype=float)

        print("[CASE23] eye-to-head rotation 제거 / translation(IPD) 유지")
        self.frustum = []
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # CASE 22 핵심 변경:
            # HMD forward(yaw/pitch)는 그대로 사용하고,
            # right / up을 cross product로 명시적으로 재구성한다.
            forward = -axes[:, 2]
            ...
            # 거의 수직으로 위/아래를 볼 때는 HMD-derived up으로 fallback.
            if right_norm < 1e-6:
                fallback_up = axes[:, 1]
                fallback_up = fallback_up - forward * np.dot(fallback_up, forward)
                fallback_up = fallback_up / max(np.linalg.norm(fallback_up), 1e-12)
                right_vec = np.cross(forward, fallback_up)
                right_norm = np.linalg.norm(right_vec)

            right_vec = right_vec / max(right_norm, 1e-12)

            # up = right x forward
            up = np.cross(right_vec, forward)
...
def main():
    ...
    print("=" * 72)
    print("CASE 23: EXPLICIT BASIS + EYE TRANSLATION ONLY")
    print("BASE = CASE 22 explicit orthonormal camera basis")
    print("추가 변경 = eye-to-head rotation 제거 / translation(IPD) 유지")
    print("CASE 22와 distortion / stereo / comfort를 직접 비교하세요.")
    print("=" * 72)
