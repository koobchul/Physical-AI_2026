"""
CASE 30: ROLL-ONLY + FIXED FORWARD
DATE: 2026-09-18
BASE: case_13_world_up_lock_no_roll.py

CASE 28(yaw-only), CASE 29(pitch-only)에 이어
HMD rotation의 roll 성분만 따로 분리하기 위한 진단 실험.

배경:
    - CASE 09: HMD camera override를 끄면 distortion이 사라짐.
    - CASE 13: world-up + no-roll에서 체감 안정성이 크게 개선됨.
      -> roll / up-vector 처리가 문제에 기여할 가능성이 큼.
    - CASE 28: yaw-only 진단.
    - CASE 29: pitch-only 진단.
    - 따라서 이번에는 yaw/pitch를 고정하고 roll만 camera에 반영해
      roll 단독으로 world tilt / distortion을 만드는지 확인한다.

목적:
    첫 유효 pose에서 camera forward를 기준 방향으로 저장하고,
    이후 HMD의 roll angle만 추출해 그 fixed forward 축 주위로 up vector를 회전한다.
...
이번 케이스에서 바꾼 것:
    - CASE 13의 no-roll을 해제하되,
      yaw/pitch는 시작 방향으로 고정하고 roll만 반영.
    - position / IPD / projection / timing은 CASE 13 그대로 유지.

그대로 유지한 것:
    - baseline head translation lock
    - eye-to-head transform / IPD
    - SteamVR projection frustum
    - HMD pose source / timing
    ...
    - controller teleoperation / physics 구조

내일 확인할 것:
    1. 머리를 좌우로 기울일 때 virtual world가 얼마나 같이 기울어지는가?
    2. roll만으로 floor / table edge가 휘거나 swimming하는가?
    3. CASE 13(no-roll) 대비 discomfort가 즉시 커지는가?
    4. 작은 roll에서도 geometry distortion이 커지는가?
    5. yaw/pitch를 움직여도 virtual camera forward가 거의 고정되는가?
       -> 이건 정상 동작이며 진단 목적.

판정:
    - roll-only에서 distortion / discomfort가 명확히 재현:
        roll/up-vector 처리 문제가 핵심 원인 중 하나라는 강한 증거.
    - roll-only는 비교적 안정적:
        CASE 13 개선은 roll 제거 자체보다 yaw/pitch와 roll의 coupling을
        끊은 효과였을 가능성.
    - yaw/pitch-only는 안정적인데 roll-only만 나쁨:
        roll 처리 우선 수정.
    - 세 축 단독은 모두 안정적인데 조합에서만 문제:
        rotation composition / camera-frame coupling이 핵심 원인 후보.

안전:
    이 Case는 의도적으로 roll을 다시 camera에 반영한다.
    멀미 가능성이 있으므로 처음에는 머리를 5~10도 정도만 천천히 기울이고,
    불편하면 즉시 종료한다.
"""
...
class EyeRig:
    def __init__(self, vr_system, roll_test=0.0, lock_translation=True):
        ...
        # CASE 30: 각 eye별 시작 forward / no-roll up 기준.
        self._roll_only_forward_ref = [None, None]
        self._roll_only_up_ref = [None, None]
        self.eye_to_head = [
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # 현재 HMD/eye orientation에서 raw forward/up을 얻는다.
            forward_raw = -axes[:, 2]
            forward_raw = forward_raw / max(np.linalg.norm(forward_raw), 1e-12)

            up_raw = axes[:, 1]
            up_raw = up_raw - forward_raw * np.dot(up_raw, forward_raw)
            up_raw = up_raw / max(np.linalg.norm(up_raw), 1e-12)

            world_up = np.array([0.0, 0.0, 1.0], dtype=float)

            # CASE 30 핵심 변경:
            # 시작 시점의 forward를 고정하고, 이후 roll angle만 적용한다.
            if self._roll_only_forward_ref[i] is None:
                forward_ref = forward_raw.copy()

                up_ref = world_up - forward_ref * np.dot(world_up, forward_ref)
                up_ref_norm = np.linalg.norm(up_ref)
                if up_ref_norm < 1e-6:
                    up_ref = up_raw.copy()
                    up_ref_norm = np.linalg.norm(up_ref)
                up_ref = up_ref / max(up_ref_norm, 1e-12)

                self._roll_only_forward_ref[i] = forward_ref
                self._roll_only_up_ref[i] = up_ref

                if i == 0:
                    print("[CASE30] fixed forward / no-roll up 기준 저장")

            forward = self._roll_only_forward_ref[i].copy()
            up_ref = self._roll_only_up_ref[i].copy()

            # 현재 raw pose의 roll만 추출.
            roll_deg = self.roll_of(forward_raw, up_raw)
            if not np.isfinite(roll_deg):
                roll_deg = 0.0

            # fixed forward 축 주위로 reference up을 roll만큼 회전.
            up = R.from_rotvec(
                forward * np.radians(roll_deg)
            ).apply(up_ref)
...
def main():
    ...
    # CASE 30은 실제 HMD roll-only 진단이므로 외부 roll-test 주입은 비활성화한다.
    args.roll_test = 0.0
    ...
    print("=" * 72)
    print("CASE 30: ROLL-ONLY + FIXED FORWARD")
    print("BASE = CASE 13")
    print("변경 = yaw/pitch 시작값 고정 / 실제 HMD roll만 camera에 적용")
    print("※ 처음에는 머리를 5~10도 정도만 천천히 기울이세요.")
    print("=" * 72)
