"""
CASE 27: NO-ROLL + INVERSE HMD ROTATION CONVENTION
DATE: 2026-09-18
BASE: case_13_world_up_lock_no_roll.py

09/17 결과에서 가장 중요한 남은 가설 중 하나인
"HMD pose rotation convention이 반대로 해석되고 있는가?"를 직접 검사한다.

배경:
    - CASE 09: HMD camera override를 끄면 distortion이 사라짐.
      -> 문제는 HMD pose를 MuJoCo camera에 적용하는 경로에 강하게 연관됨.
    - CASE 13: world-up + no-roll에서 크게 안정화됨.
      -> roll/up-vector 문제는 분명 일부 존재함.
    - 그러나 residual distortion / world swimming은 남았음.
    - OpenVR tracking pose는 device-to-absolute transform으로 제공되므로,
      camera pose / view convention을 잘못 해석하면 회전 방향이나 축 coupling이
      잘못될 수 있음.

목적:
    CASE 13의 world-up/no-roll 설정은 그대로 유지하면서,
    HMD rotation matrix만 transpose(= inverse, rotation matrix 기준)하여 사용한다.
    ...
    position / IPD / projection / timing은 건드리지 않는다.

이번 케이스에서 바꾼 것:
    - CASE 13 대비 딱 하나:
      SVD로 orthonormalize한 HMD rotation을 transpose해서 사용.
    - HMD translation은 CASE 13과 동일하게 시작 위치에 고정.
    - eye-to-head transform은 그대로 유지.
...
내일 확인할 것:
    1. 머리를 오른쪽으로 돌릴 때 virtual camera가 자연스러운 방향으로 반응하는가?
    2. 왼쪽/오른쪽 yaw가 반대로 움직이는가?
    3. pitch up/down이 반대로 움직이는가?
    4. CASE 13보다 world tilt / swimming / distortion이 줄어드는가?
    5. yaw와 pitch의 coupling이 줄어드는가?
    6. horizon 안정성은 유지되는가?

판정:
    - 회전 방향이 명백히 반대이고 더 불편:
        현재 device-to-absolute orientation 사용 방향은 대체로 맞다는 증거.
        이 경우 CASE 27은 즉시 중단해도 됨.
    - 방향은 자연스럽고 distortion이 크게 개선:
        camera/view rotation convention을 반대로 해석했을 가능성이 매우 큼.
    - 방향은 자연스럽지만 distortion 동일:
        rotation convention 자체보다 coordinate-axis mapping을 더 봐야 함.

안전:
    이 Case는 회전 방향이 반대로 느껴질 수 있다.
    VR 멀미 가능성이 있으므로 처음에는 아주 작은 yaw/pitch만 천천히 시험하고,
    반대로 움직이면 오래 착용하지 말고 즉시 종료한다.
"""
...
class EyeRig:
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        # CASE 27 핵심 변경:
        # OpenVR의 device-to-absolute rotation을 반대 convention으로 해석하는
        # 가설을 검사하기 위해 rotation만 inverse (= transpose)해서 사용한다.
        # translation / IPD / projection은 CASE 13 그대로 유지한다.
        hmd[:3, :3] = rot.T
        ...
        for i in range(2):
            ...
            # CASE 27에서도 CASE 13의 world-up/no-roll을 그대로 유지:
            # inverse HMD rotation에서 얻은 forward(yaw/pitch)를 사용하고,
            # up은 월드 +Z 기준으로 재구성해 camera roll을 제거한다.
...
def main():
    ...
    print("=" * 72)
    print("CASE 27: NO-ROLL + INVERSE HMD ROTATION")
    print("BASE = CASE 13 world-up/no-roll")
    print("추가 변경 = HMD rotation R -> R.T (inverse convention test)")
    print("※ 아주 작은 yaw/pitch부터 천천히 확인하세요. 반대면 즉시 종료.")
    print("=" * 72)
