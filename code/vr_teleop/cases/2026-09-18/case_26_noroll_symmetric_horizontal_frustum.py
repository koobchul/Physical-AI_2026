"""
CASE 26: NO-ROLL + SYMMETRIC HORIZONTAL FRUSTUM CENTER
DATE: 2026-09-18
BASE: case_13_world_up_lock_no_roll.py

09/17 결과에서 camera-frame 문제와 projection 문제를 분리해 다시 확인하는 실험.

배경:
    - CASE 13: world-up + no-roll에서 motion comfort와 안정성이 크게 개선됨.
    - CASE 03: baseline 계열에서 horizontal frustum center를 0으로 만들어도
      CASE 02와 거의 비슷하게 보였음.
    - 하지만 당시에는 camera roll / world-up 문제가 그대로 남아 있었기 때문에,
      projection center 효과가 motion distortion에 가려졌을 가능성이 있음.

목적:
    CASE 13의 안정된 world-up/no-roll camera orientation은 그대로 유지하고,
    horizontal asymmetric frustum center만 0으로 만든다.
    ...
    horizontal FOV span과 frustum_width는 그대로 두고,
    optical center의 좌우 비대칭만 제거한다.

이번 케이스에서 바꾼 것:
    - CASE 13 대비 딱 하나:
      horizontal frustum_center = 0.0
    - vertical frustum top / bottom은 그대로.
    - render resolution / aspect도 그대로.
...
내일 확인할 것:
    1. CASE 13보다 yaw 시 좌우 공간 휨이 줄어드는가?
    2. 화면 중앙 object가 좌우 눈에서 더 자연스럽게 정렬되는가?
    3. horizontal double vision / cross-eyed 느낌이 줄어드는가?
    4. floor / table edge가 좌우 회전에서 덜 휘는가?
    5. stereo depth가 무너지지 않는가?
    6. CASE 13의 motion comfort / latency가 유지되는가?

판정:
    - CASE 26이 CASE 13보다 확실히 개선:
        horizontal off-axis center가 residual distortion에 기여했을 가능성.
    - geometry는 좋아지지만 stereo가 불편:
        HMD optics에는 asymmetric center가 필요하되 현재 적용 방식에
        추가 조정이 필요할 가능성.
    - 거의 동일:
        horizontal frustum center는 주원인이 아닐 가능성.
    - 더 나빠짐:
        원래 SteamVR horizontal asymmetry가 실제 stereo optics에 필요한 값일 가능성.

주의:
    이 실험은 horizontal FOV 크기를 바꾸는 것이 아니라
    horizontal optical center만 0으로 만드는 진단 실험이다.
"""
...
class EyeRig:
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # CASE 26에서도 CASE 13 orientation을 그대로 유지:
            # HMD가 정한 forward(yaw/pitch)는 그대로 쓰되,
            # up은 월드 +Z를 forward에 직교 투영해 만들어 camera roll을 제거한다.
            ...
            # CASE 26 핵심 변경:
            # horizontal FOV span은 그대로 두고 optical center만 0으로 만든다.
            cam.frustum_center = 0.0
            try:
...
def main():
    ...
    print("=" * 72)
    print("CASE 26: NO-ROLL + SYMMETRIC HORIZONTAL FRUSTUM")
    print("BASE = CASE 13 world-up/no-roll")
    print("추가 변경 = horizontal frustum_center = 0.0")
    print("CASE 13과 yaw distortion / stereo center를 직접 비교하세요.")
    print("=" * 72)
