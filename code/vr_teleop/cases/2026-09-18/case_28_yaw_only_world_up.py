"""
CASE 28: YAW-ONLY + WORLD-UP LOCK
DATE: 2026-09-18
BASE: case_13_world_up_lock_no_roll.py

09/17 결과에서 남은 distortion이 yaw 자체에서 생기는지,
아니면 pitch가 섞일 때 생기는지 분리하기 위한 진단 실험.

배경:
    - CASE 09: HMD camera override를 끄면 distortion이 사라짐.
    - CASE 13: world-up + no-roll에서 안정성이 크게 좋아졌지만
      residual distortion은 남았음.
    - 따라서 HMD orientation의 yaw / pitch 중 어떤 성분이
      distortion에 더 직접적으로 연결되는지 분리할 필요가 있음.

목적:
    CASE 13의 world-up/no-roll은 유지하면서,
    HMD forward에서 pitch 성분을 제거하고 yaw만 camera에 적용한다.
    ...
    머리를 좌우로 돌릴 때 distortion이 남는지 확인한다.

이번 케이스에서 바꾼 것:
    - CASE 13 대비 camera forward의 Z 성분을 0으로 만들어
      horizontal plane에 projection.
    - 따라서 camera orientation은 yaw-only.
...
내일 확인할 것:
    1. 머리를 좌우(yaw)로 돌릴 때 world tilt / bending이 남는가?
    2. floor / table edge가 yaw에서 여전히 휘는가?
    3. CASE 13보다 좌우 회전이 더 안정적인가?
    4. 머리를 위/아래로 움직여도 화면 pitch가 고정되는가?
       -> 이건 정상 동작이며 진단 목적.
    5. yaw-only 상태에서도 vertical narrowing은 그대로 남는가?
    6. latency / buffering은 CASE 13과 비슷한가?

판정:
    - yaw-only에서도 distortion이 그대로 남음:
        yaw / horizontal axis mapping 또는 projection 문제가 더 강한 후보.
    - yaw-only에서 distortion이 크게 감소:
        pitch 성분 또는 yaw-pitch coupling이 원인일 가능성이 큼.
    - yaw는 안정적이지만 pitch를 제거한 것 자체가 불편:
        정상이며, 이 Case는 최종 구현이 아니라 진단용.

주의:
    이 Case에서는 고개를 위/아래로 숙여도 virtual camera가 따라가지 않는다.
    반드시 먼저 작은 좌우 yaw만 천천히 테스트한다.
"""
...
class EyeRig:
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # CASE 28 핵심 변경:
            # HMD forward를 robot-world horizontal plane(XY)에 투영하여
            # pitch를 제거하고 yaw만 camera orientation에 반영한다.
            forward = forward_raw.copy()
            forward[2] = 0.0
            forward_norm = np.linalg.norm(forward)

            # 위/아래를 거의 수직으로 보고 있어 horizontal projection이 퇴화하면
            # 직전 안정 방향 대신 eye X/Y basis에서 horizontal forward를 복구한다.
            if forward_norm < 1e-6:
                candidate = -axes[:, 2].copy()
                candidate[2] = 0.0
                forward_norm = np.linalg.norm(candidate)
                if forward_norm < 1e-6:
                    candidate = np.array([1.0, 0.0, 0.0], dtype=float)
                    forward_norm = 1.0
                forward = candidate

            forward = forward / max(forward_norm, 1e-12)

            # Yaw-only이므로 up은 항상 world +Z.
...
def main():
    ...
    # CASE 28은 yaw-only 진단이므로 외부 roll-test 주입은 비활성화한다.
    args.roll_test = 0.0
    ...
    print("=" * 72)
    print("CASE 28: YAW-ONLY + WORLD-UP LOCK")
    print("BASE = CASE 13")
    print("변경 = camera pitch 제거 / yaw만 적용 / roll도 제거")
    print("※ 작은 좌우 yaw부터 테스트하세요. 위/아래 pitch가 안 움직이는 게 정상입니다.")
    print("=" * 72)
