"""
Oculus Quest 2 -> robosuite teleoperation (OpenVR Scene 앱 / 머리 트래킹 스테레오)
...
# --------------------------------------------------------------------------
class EyeRig:
    def __init__(self, vr_system, roll_test=0.0, fov_scale=1.0,
                 fov_scale_x=1.0, fov_scale_y=1.0):
        self.vr = vr_system
        self.roll_test = roll_test
        self.fov_scale = fov_scale
        self.sx = fov_scale * fov_scale_x
        self.sy = fov_scale * fov_scale_y
        self.eye_to_head = [
        ...
        for eye in (openvr.Eye_Left, openvr.Eye_Right):
            ...
            # 보고된 화각이 실제 표시 화각과 다를 수 있다. 축별 배율로 보정.
            cx, cy = (left + right) / 2, (bottom + top) / 2
            left = cx + (left - cx) * self.sx
            right = cx + (right - cx) * self.sx
            bottom = cy + (bottom - cy) * self.sy
            top = cy + (top - cy) * self.sy
    ...
    def eye_params(self, hmd_pose_vr, origin, i):
        """i번 눈의 (위치, forward, up)을 로봇 좌표로 반환."""
        eye_world_vr = hmd_pose_vr @ self.eye_to_head[i]
        pos = VR2ROBOT @ eye_world_vr[:3, 3] + origin
        # R_vr의 열 = 눈 로컬 축을 VR 월드로 표현한 것.
        # 로봇 좌표로 옮기려면 왼쪽 곱 한 번뿐이다.
        # (C R C^T 상사변환은 상대회전용이고 여기선 틀린다.)
        axes = VR2ROBOT @ eye_world_vr[:3, :3]
        forward = -axes[:, 2]     # GL 카메라는 로컬 -Z를 바라본다
        up = axes[:, 1]
        if self.roll_test:
            up = R.from_rotvec(forward * np.radians(self.roll_test)).apply(up)
        return pos, forward, up

    def _fill(self, cam, pos, forward, up, i):
        ...
        try:
            ...
            pass   # 구버전 MuJoCo: aspect로 폭이 결정됨

    def apply_single(self, scn, hmd_pose_vr, origin, i):
        """두 카메라 슬롯을 모두 i번 눈으로 채운다.
        모노(mjSTEREO_NONE)로 한 눈씩 따로 렌더할 때 사용."""
        pos, forward, up = self.eye_params(hmd_pose_vr, origin, i)
        for slot in range(2):
            self._fill(scn.camera[slot], pos, forward, up, i)
        try:
            if scn.nlight > 0:
                scn.lights[0].pos[:] = pos.astype(np.float32)
                scn.lights[0].dir[:] = forward.astype(np.float32)
        except Exception:
            pass

    def apply(self, scn, hmd_pose_vr, origin):
        """scn.camera[0..1]을 실제 눈 위치/투영으로 덮어쓴다 (SBS용)."""
        mid = np.zeros(3)
        fwd_mid = np.zeros(3)

        for i in range(2):
            pos, forward, up = self.eye_params(hmd_pose_vr, origin, i)
            self._fill(scn.camera[i], pos, forward, up, i)
...
def parse_args():
    ...
    p.add_argument("--two-pass", action="store_true",
                   help="MuJoCo의 side-by-side를 쓰지 않고 각 눈을 모노로 "
                        "따로 렌더한다. 종횡비가 반쪽 뷰포트로 명확해진다")
    p.add_argument("--width-scale", type=float, default=1.0,
                   help="눈당 렌더 가로 픽셀수 배율 = 뷰포트 종횡비 변경. "
                        "MuJoCo가 종횡비로 가로 화각을 유도하는지 시험용. 0.5부터")
    p.add_argument("--fov-scale-x", type=float, default=1.0,
                   help="frustum 가로 화각만 배율. frustum_width를 쓰는지 시험용")
    p.add_argument("--fov-scale-y", type=float, default=1.0,
                   help="frustum 세로 화각만 배율")
    p.add_argument("--fov-scale", type=float, default=1.0,
                   help="frustum 화각 배율. ALVR이 보고하는 화각이 실제와 다를 때 "
                        "0.8~0.9로 좁히거나 1.1로 넓혀 본다")
...
def main():
    ...
    eyes = EyeRig(vr_system, roll_test=args.roll_test,
                  ...
                  fov_scale_x=args.fov_scale_x,
                  fov_scale_y=args.fov_scale_y)
    ...
    if args.eye_width > 0 and args.eye_height > 0:
        ...
        EW = max(256, int(round(EH * aspect * args.width_scale)) // 8 * 8)
    max_total_w = 16384          # 렌더버퍼 보수적 상한 (GL 컨텍스트 생성 전)
    if EW * 2 > max_total_w:
        EW = (max_total_w // 2) // 8 * 8
        print(f"[VR] 가로가 상한을 넘어 {EW}로 제한 (전체 {EW*2})")
    ...
    scn.stereo = (mujoco.mjtStereo.mjSTEREO_NONE if args.two_pass
                  else mujoco.mjtStereo.mjSTEREO_SIDEBYSIDE)
    print(f"[MJ] stereo 모드 = {scn.stereo} "
          f"({'two-pass mono' if args.two_pass else 'side-by-side'})")
    ...
    try:
        while True:
            ...
            hmd_m = (mat34_to_numpy(hmd_pose.mDeviceToAbsoluteTracking)
                     if hmd_pose.bPoseIsValid else None)
            if hmd_m is not None and not args.no_vr_cam and not args.two_pass:
                eyes.apply(scn, hmd_m, origin)
            ...
            _t = time.time()
            if args.two_pass and hmd_m is not None and not args.no_vr_cam:
                # 각 눈을 모노로 반쪽 뷰포트에 따로 렌더
                for i in range(2):
                    eyes.apply_single(scn, hmd_m, origin, i)
                    mujoco.mjr_render(
                        mujoco.MjrRect(i * EW, 0, EW, EH), scn, con)
            else:
