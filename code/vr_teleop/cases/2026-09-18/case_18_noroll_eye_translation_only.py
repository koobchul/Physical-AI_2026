...
오늘(09/17) 결과에서 출발한 첫 번째 2차 진단 실험.

배경:
    - CASE 09: HMD camera override를 끄면 distortion이 사라짐.
      -> 문제 영역이 HMD pose -> MuJoCo camera 적용 과정으로 좁혀짐.
    - CASE 10: eye-to-head rotation을 제거하고 translation(IPD)만 남겼을 때
      high resolution / no buffering / preferred로 기록됨.
    - CASE 13: world-up 고정 + no-roll에서 가장 안정적인 체감 중 하나였음.
      다만 distortion과 화면 비율 문제는 남아 있었음.

목적:
    CASE 13의 no-roll/world-up 안정화는 그대로 유지하면서,
    eye-to-head transform의 회전 성분(cant)을 제거한다.

    즉,
        HMD rotation            : 사용
        HMD roll camera 반영    : 제거 (CASE 13 방식)
        eye translation / IPD   : 유지
        eye-to-head rotation    : 제거

    이 조합에서 CASE 13보다 distortion이 더 줄어드는지 확인한다.

이번 케이스에서 바꾼 것:
    - CASE 13 대비 딱 하나:
      각 eye_to_head transform의 3x3 rotation을 identity로 교체.
    - 각 눈의 translation(IPD)은 그대로 유지.
...
그대로 유지한 것:
    ...
    - CASE 13 world-up lock / camera roll 제거
    - SteamVR projection frustum
    - native render-target resolution / aspect
    - MuJoCo stereo side-by-side
    - OpenGL FBO / GPU blit
    - OpenVR IVRCompositor::Submit
    - physics / render loop 구조

내일 확인할 것:
    1. CASE 13보다 head yaw/pitch 때 world distortion이 줄어드는가?
    2. floor / table / robot geometry가 머리 움직임에 따라 덜 휘는가?
    3. stereo depth는 정상적으로 유지되는가?
    4. double vision / eye strain이 증가하지 않는가?
    5. CASE 10에서 보였던 horizontally narrower 현상이 남는가?
    6. latency / buffering은 CASE 13 수준으로 유지되는가?

판정:
    - CASE 13보다 확실히 개선:
        eye-to-head rotation(cant)이 남은 distortion에 기여했을 가능성 큼.
    - 거의 동일:
        eye rotation은 핵심이 아니고 HMD camera frame 자체를 더 파야 함.
    - 더 나빠짐:
        eye rotation이 projection과 함께 필요한 요소일 수 있음.

주의:
    이 파일은 진단용이다.
    눈이 불편하거나 stereo가 이상하면 오래 보지 말고 즉시 종료한다.
"""
...
class EyeRig:
    def __init__(self, vr_system, roll_test=0.0, lock_translation=True):
        ...
        # CASE 18 핵심 변경:
        # CASE 13의 world-up / no-roll 처리는 그대로 유지하고,
        # eye-to-head의 translation(IPD)은 보존한 채 rotation(cant)만 제거한다.
        ...
        print("[CASE18] eye-to-head rotation 제거 / translation(IPD) 유지")
        self.frustum = []
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # CASE 18은 CASE 13의 핵심 설정을 그대로 유지:
...
def main():
    ...
    print("=" * 72)
    print("CASE 18: NO-ROLL + EYE TRANSLATION ONLY")
    print("BASE = CASE 13 world-up/no-roll")
    print("추가 변경 = eye-to-head rotation 제거, translation(IPD)은 유지")
    print("CASE 13과 distortion / stereo / latency를 직접 비교하세요.")
    print("=" * 72)
