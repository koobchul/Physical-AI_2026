"""
CASE 21: NO-ROLL + FULL 6DoF HEAD TRANSLATION
DATE: 2026-09-18
BASE: case_13_world_up_lock_no_roll.py

09/17 결과를 바탕으로 한 2차 world-lock 진단 실험.

배경:
    - CASE 09: HMD camera override를 끄면 distortion이 사라짐.
    - CASE 13: world-up 고정 + no-roll에서 체감 안정성이 크게 개선됨.
    - 하지만 CASE 13은 baseline과 동일하게 HMD translation을 시작 위치에 고정함.
    - 실제 head rotation에는 작은 XYZ translation이 자연스럽게 동반될 수 있음.

목적:
    CASE 13의 world-up/no-roll orientation은 그대로 유지하면서,
    HMD XYZ translation lock만 해제하여 full 6DoF를 사용한다.

    no-roll로 회전 안정성을 확보한 상태에서 positional tracking까지 살렸을 때
    world-lock / parallax / floor swimming이 개선되는지 확인한다.

이번 케이스에서 바꾼 것:
    - CASE 13 대비 딱 하나:
      args.full_6dof = True
    - 따라서 EyeRig(lock_translation=False)로 동작하며
      HMD의 live XYZ translation을 camera position에 반영한다.
...
내일 확인할 것:
    1. 머리를 좌우로 5~15 cm 움직일 때 자연스러운 parallax가 생기는가?
    2. 머리를 앞뒤로 움직일 때 table / robot / floor가 더 잘 고정되는가?
    3. 회전 시 동반되는 작은 translation 때문에 생기던 swimming이 줄어드는가?
    4. translation scale이 너무 크거나 작지 않은가?
    5. forward/backward 또는 좌우 axis가 뒤집히지 않는가?
    6. CASE 13보다 distortion / comfort가 좋아지는가?

판정:
    - CASE 21이 CASE 13보다 확실히 자연스러움:
        translation lock이 residual world-swimming에 기여했을 가능성이 큼.
    - 회전은 좋지만 translation 방향/scale이 이상함:
        HMD position coordinate transform 또는 scale을 다음에 집중 점검.
    - 거의 동일:
        translation lock은 핵심 원인이 아닐 가능성이 큼.
    - 더 나빠짐:
        live translation transform / tracking noise / scale 문제가 있을 가능성.

중요:
    이 Case는 반드시 고개 회전만 하지 말고
    머리를 실제로 좌우, 앞뒤로 5~15 cm 이동하면서 비교한다.
"""
...
class EyeRig:
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # CASE 21에서도 CASE 13 orientation을 그대로 유지:
...
def main():
    ...
    # CASE 21 핵심 변경:
    # CASE 13의 world-up/no-roll orientation은 유지하면서
    # HMD translation lock만 해제해 live XYZ translation을 사용한다.
    args.full_6dof = True
    ...
    print("=" * 72)
    print("CASE 21: NO-ROLL + FULL 6DoF HEAD TRANSLATION")
    print("BASE = CASE 13 world-up/no-roll")
    print("추가 변경 = HMD translation lock OFF / live XYZ translation")
    print("※ 반드시 머리를 좌우/앞뒤로 5~15 cm 실제 이동하며 비교하세요.")
    print("=" * 72)
