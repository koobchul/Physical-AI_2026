"""
CASE 25: NO-ROLL + SYMMETRIC VERTICAL FRUSTUM
DATE: 2026-09-18
BASE: case_13_world_up_lock_no_roll.py

09/17 결과에서 남은 "vertically narrower / vertical geometry" 문제를
조금 더 직접적으로 분리하기 위한 projection 진단 실험.

배경:
    - CASE 13: world-up + no-roll에서 motion stability는 크게 좋아졌지만
      화면이 vertically narrower하게 보이는 문제가 남았음.
    - CASE 15: vertical frustum sign 자체를 뒤집는 방식은 distortion과
      high latency를 남겨 좋은 해결책이 아니었음.
    - CASE 24: render aspect를 projection frustum aspect로 맞추는 실험.
    - 따라서 vertical projection의 "부호"가 아니라
      vertical optical-center asymmetry 자체가 문제인지 따로 볼 필요가 있음.

목적:
    CASE 13의 camera orientation / world-up / no-roll은 그대로 유지하면서,
    각 eye의 vertical frustum span은 보존하되 중심만 0으로 맞춘다.

    baseline:
        frustum_bottom = bottom * NEAR
        frustum_top    = top * NEAR

    CASE 25:
        vertical_span = (top - bottom) * NEAR
        frustum_bottom = -0.5 * vertical_span
        frustum_top    = +0.5 * vertical_span

    즉 vertical FOV 크기는 유지하고,
    위/아래 비대칭 projection offset만 제거한다.

이번 케이스에서 바꾼 것:
    - CASE 13 대비 vertical frustum center만 대칭화.
    - horizontal frustum center / width는 그대로.
    - render resolution / aspect는 그대로.
    - HMD pose / eye transform / IPD도 그대로.
...
내일 확인할 것:
    1. CASE 13의 vertically narrower 느낌이 줄어드는가?
    2. horizon의 상/하 위치가 더 자연스러운가?
    3. head pitch 시 vertical geometry bending이 줄어드는가?
    4. floor / table 높이감이 더 자연스러운가?
    5. stereo eye alignment가 나빠지지 않는가?
    6. CASE 13의 motion comfort는 유지되는가?

판정:
    - CASE 25가 화면비/vertical geometry만 개선:
        vertical optical-center asymmetry가 별도 시각 문제였을 가능성.
    - distortion까지 같이 개선:
        vertical projection offset이 motion distortion에도 기여했을 가능성.
    - 더 나빠짐 / eye strain 증가:
        원래 SteamVR vertical asymmetry가 실제 HMD optics에 필요한 값일 가능성.
    - 차이 없음:
        vertical narrowing의 원인은 vertical center가 아닐 가능성.

주의:
    이 파일은 vertical FOV 크기를 바꾸는 것이 아니라
    동일한 vertical span을 0 중심으로 재배치하는 진단 실험이다.
"""
...
class EyeRig:
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # CASE 25에서도 CASE 13 orientation을 그대로 유지:
            ...
            # CASE 25 핵심 변경:
            # vertical FOV span은 보존하고, vertical optical center만 0으로 맞춘다.
            vertical_span = (top - bottom) * NEAR
            cam.frustum_bottom = -0.5 * vertical_span
            cam.frustum_top = 0.5 * vertical_span

            # horizontal projection은 CASE 13 / baseline 그대로 유지.
...
def main():
    ...
    print("=" * 72)
    print("CASE 25: NO-ROLL + SYMMETRIC VERTICAL FRUSTUM")
    print("BASE = CASE 13 world-up/no-roll")
    print("추가 변경 = vertical FOV span 유지 + vertical center만 0으로 대칭화")
    print("CASE 13과 vertical narrowing / pitch distortion을 직접 비교하세요.")
    print("=" * 72)
