...
09/17 결과를 바탕으로 한 2차 camera-frame 진단 실험.

배경:
    - CASE 09: HMD camera override를 끄면 distortion이 사라짐.
    - CASE 13: world-up 고정 + no-roll에서 체감이 크게 개선됨.
    - 따라서 HMD orientation을 MuJoCo camera의 forward / up basis로
      바꾸는 과정 자체가 핵심 원인 후보로 남음.

목적:
    CASE 13과 동일하게 HMD의 forward(yaw/pitch)는 사용하고 roll은 제거하되,
    camera right / up basis를 cross product로 명시적으로 다시 만든다.

    CASE 13:
        world_up을 forward에 직교 투영하여 up 계산

    CASE 22:
        right = normalize(cross(forward, world_up))
        up    = normalize(cross(right, forward))

    forward / right / up을 명시적인 orthonormal basis로 만들어
    yaw / pitch 중 camera-frame coupling이 distortion에 영향을 주는지 확인한다.

이번 케이스에서 바꾼 것:
    - CASE 13 대비 camera basis 구성 방식만 변경.
    - HMD forward는 그대로 사용.
    - camera roll은 계속 제거.
    - world +Z를 기준 up으로 사용.
...
내일 확인할 것:
    1. CASE 13보다 yaw 시 horizon / floor가 덜 비트는가?
    2. pitch 시 table / floor geometry가 덜 휘는가?
    3. yaw와 pitch가 서로 섞이는 느낌이 줄어드는가?
    4. 빠른 head rotation에서 world swimming이 줄어드는가?
    5. vertical narrowing / stretching 체감이 달라지는가?
    6. latency는 CASE 13과 비슷하게 유지되는가?

판정:
    - CASE 22가 CASE 13보다 확실히 개선:
        camera basis 구성 방식이 residual distortion에 기여했을 가능성 큼.
    - 거의 동일:
        CASE 13 basis는 이미 충분히 안정적이며 다른 원인을 더 봐야 함.
    - 더 나빠짐:
        explicit basis 방식이 현재 coordinate convention과 맞지 않을 가능성.

주의:
    forward가 world-up과 거의 평행할 때 cross product가 작아질 수 있어
    그 경우에는 HMD-derived up으로 fallback basis를 만든다.
"""
...
def main():
    ...
    print("=" * 72)
    print("CASE 22: EXPLICIT ORTHONORMAL CAMERA BASIS")
    print("BASE = CASE 13 world-up/no-roll")
    print("변경 = right/up을 cross product로 명시적으로 재구성")
    print("CASE 13과 yaw/pitch distortion / world-lock을 직접 비교하세요.")
    print("=" * 72)
