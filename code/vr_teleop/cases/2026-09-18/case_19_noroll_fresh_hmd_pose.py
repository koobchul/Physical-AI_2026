...
오늘(09/17) 결과를 바탕으로 한 2차 진단 실험.

배경:
    - CASE 09: HMD camera override를 끄면 distortion이 사라짐.
      -> 문제 영역이 HMD pose -> MuJoCo camera 적용 과정으로 좁혀짐.
    - CASE 13: world-up 고정 + no-roll에서 체감이 크게 개선됨.
      -> roll / up-vector / camera-frame 처리가 중요한 원인 후보.
    - CASE 08: predicted pose에서 latency는 일부 개선되었지만
      floor/world-lock 문제는 남았음.

목적:
    CASE 13의 world-up/no-roll camera frame은 그대로 유지하면서,
    waitGetPoses()에서 받은 pose 대신 렌더 직전에 다시 읽은
    최신 0 ms HMD pose를 실제 MuJoCo camera에 사용한다.

    이를 통해 CASE 13에 남아 있던 distortion / world swimming 중 일부가
    pose freshness / timing 문제인지 분리한다.

이번 케이스에서 바꾼 것:
    - CASE 13 대비 딱 하나:
      compositor waitGetPoses()는 동기화용으로 유지하지만,
      렌더 직전에 getDeviceToAbsoluteTrackingPose(..., 0.0, ...)를 호출해
      fresh HMD pose를 camera에 사용한다.
    - fresh pose가 invalid이거나 호출 실패 시 waitGetPoses pose로 fallback.

그대로 유지한 것:
    - CASE 13 world-up lock
    - CASE 13 camera roll 제거
    - HMD yaw / pitch
    - baseline head translation lock
...
내일 확인할 것:
    1. CASE 13보다 머리를 돌릴 때 world가 덜 따라오는가?
    2. floor / table이 HMD와 함께 미끄러지는 느낌이 줄어드는가?
    3. 빠른 yaw / pitch에서 distortion이 감소하는가?
    4. 머리를 멈춘 직후 world가 늦게 따라오는 현상이 줄어드는가?
    5. jitter가 증가하지 않는가?
    6. latency가 CASE 13보다 좋아지는가?

판정:
    - CASE 13보다 확실히 개선:
        residual distortion에 pose freshness / timing이 기여했을 가능성이 큼.
    - latency만 개선되고 distortion 동일:
        timing은 별도 문제지만 geometry root cause는 아님.
    - 거의 동일:
        0 ms fresh pose 자체는 핵심이 아님.
    - 더 나빠짐:
        compositor가 기대하는 waitGetPoses pose와 camera pose가 어긋나는
        reprojection mismatch 가능성을 의심.

주의:
    이 파일은 진단용이다.
    최종 구현으로 확정하지 말고 CASE 13과 직접 비교한다.
"""
...
class EyeRig:
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # CASE 19에서도 CASE 13 camera frame을 그대로 유지:
...
def get_fresh_hmd_pose(vr_system, fallback_pose):
    """
    CASE 19:
    waitGetPoses()는 compositor 동기화용으로 유지하고,
    실제 camera 적용 직전에 prediction 없는 0.0 s HMD pose를 다시 읽는다.
    invalid / exception 시 waitGetPoses() pose를 사용한다.
    """
    try:
        fresh_poses = vr_system.getDeviceToAbsoluteTrackingPose(
            openvr.TrackingUniverseStanding,
            0.0,
            openvr.k_unMaxTrackedDeviceCount,
        )
        fresh = fresh_poses[openvr.k_unTrackedDeviceIndex_Hmd]
        if fresh.bPoseIsValid:
            return fresh
    except Exception as e:
        if not getattr(get_fresh_hmd_pose, "_warned", False):
            get_fresh_hmd_pose._warned = True
            print(f"[CASE19] fresh HMD pose 실패 -> waitGetPoses pose fallback: {e}")
...
def main():
    ...
    print("=" * 72)
    print("CASE 19: NO-ROLL + FRESH HMD POSE (0 ms)")
    print("BASE = CASE 13 world-up/no-roll")
    print("추가 변경 = 렌더 직전 0 ms fresh HMD pose 사용")
    print("CASE 13과 distortion / world-lock / latency를 직접 비교하세요.")
    print("=" * 72)
    ...
    try:
        while True:
            ...
            # CASE 19 핵심 변경:
            # waitGetPoses()는 frame sync에 사용하고,
            # 실제 camera에는 렌더 직전에 다시 읽은 0 ms fresh pose를 사용한다.
            hmd_pose = get_fresh_hmd_pose(vr_system, hmd_pose_wait)
