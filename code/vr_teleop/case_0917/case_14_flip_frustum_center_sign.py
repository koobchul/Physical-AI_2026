"""
CASE 14: FLIP HORIZONTAL FRUSTUM CENTER SIGN
BASE: main_vr_scene_7.py

목적:
    SteamVR의 비대칭 eye projection을 MuJoCo cam.frustum_center로 옮길 때
    좌/우 오프셋의 부호 규약이 반대로 적용되고 있는지 확인한다.

이번 케이스에서 바꾼 것:
    - baseline의 horizontal frustum center 크기는 그대로 유지
    - cam.frustum_center의 부호만 반대로 적용
      baseline : +0.5 * (left + right) * NEAR
      CASE 14  : -0.5 * (left + right) * NEAR
    - frustum width / top / bottom / eye transform / HMD pose는 그대로 유지
...
판정:
    CASE 03(frustrum center=0)에서 왜곡이 줄었다면 이 케이스가 특히 중요하다.

    - CASE 14가 baseline보다 훨씬 자연스러움:
      frustum_center의 부호 규약이 반대로 들어갔을 가능성이 높다.
    - CASE 03은 좋아졌지만 CASE 14는 더 나빠짐:
      비대칭 center 자체의 적용 방식이 문제일 가능성이 있다.
    - 둘 다 변화 없음:
      frustum center는 주원인이 아닐 가능성이 높다.

주의:
    좌/우 눈 비대칭 projection을 반대로 적용하는 진단 케이스이므로
    눈이 불편하면 오래 보지 말고 바로 종료한다.
...
class EyeRig:
    ...
    def apply(self, scn, hmd_pose_vr, origin):
        ...
        for i in range(2):
            ...
            # CASE 14 핵심 변경:
            # SteamVR <-> MuJoCo horizontal off-axis projection의
            # 부호 규약이 반대인지 확인하기 위해 center 부호만 뒤집는다.
...
def main():
    ...
    print("=" * 68)
    print("CASE 14: FLIP HORIZONTAL FRUSTUM CENTER SIGN")
    print("baseline 유지 + cam.frustum_center의 부호만 반대로 적용")
    print("CASE 03(center=0)과 함께 비교하면 원인 분리가 쉽습니다.")
    print("=" * 68)
