"""
CASE 13: WORLD-UP LOCK / CAMERA ROLL OFF
BASE: main_vr_scene_7.py

목적:
    머리를 좌우/상하로 돌릴 때 월드 전체가 같이 기울어 보이는 현상이
    HMD 회전에서 들어오는 roll 성분 또는 camera up-vector 처리 때문에 생기는지 확인한다.

이번 케이스에서 바꾼 것:
    - HMD가 결정한 camera forward 방향(yaw/pitch)은 그대로 유지
    - camera up 벡터만 월드 +Z를 기준으로 다시 계산
    - 결과적으로 camera roll을 0으로 고정
    - eye position / IPD / projection / head translation 처리 방식은 baseline 유지
...
판정:
    baseline보다 머리를 돌릴 때 월드의 기울기/비틀림이 크게 줄어들면
    roll/up-vector 처리 또는 좌표계 회전 변환이 강한 원인 후보이다.

    단, 실제로 사용자가 머리를 옆으로 기울여도 화면이 같이 기울지 않는 것이
    이 케이스의 의도된 동작이다. 최종 구현이 아니라 원인 분리용 테스트다.
...
def main():
    ...
    print("=" * 68)
    print("CASE 13: WORLD-UP LOCK / CAMERA ROLL OFF")
    print("baseline 유지 + forward(yaw/pitch)는 유지, camera roll만 0으로 고정")
    print("※ 머리를 옆으로 기울여도 화면이 같이 기울지 않는 것이 정상입니다.")
    print("=" * 68)
