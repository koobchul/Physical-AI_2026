"""
CASE 16: EXACT STEAMVR PROJECTION ASPECT
BASE: main_vr_scene_7.py

목적:
    SteamVR eye projection frustum의 실제 aspect ratio와
    MuJoCo eye render viewport aspect ratio를 최대한 정확히 일치시킨다.

현재 baseline 로그:
    SteamVR recommended eye target : 1516 x 1652
    Current render                 : 680 x 736
    Render aspect                  : 0.9239
    Projection frustum aspect      : 0.9340

기존 Case 02의 문제:
    EH * aspect 값을 만든 뒤 "// 8 * 8"로 내림 처리되어
    687.x가 다시 680으로 떨어질 수 있었다.
    따라서 baseline과 사실상 같은 680x736이 되었을 가능성이 있다.

이번 케이스에서 바꾼 것:
    - projection frustum aspect 사용을 강제
    - eye width를 가장 가까운 8 pixel 배수로 반올림
    - 현재 로그 기준 예상값: 688 x 736 (aspect ~= 0.9348)

그대로 유지한 것:
    - robosuite Lift / XArm7
    - BASIC controller / env.step()
    - HMD pose / translation lock
    - eye-to-head transform / IPD
    - roll / pitch / yaw 처리
    - projection frustum 값 자체
    - MuJoCo stereo side-by-side render
    - OpenGL FBO/blit
    - OpenVR compositor Submit
    - physics / input / render loop

판정:
    평평한 벽/직사각형을 정면으로 본 뒤
    roll left -> center -> roll right를 수행한다.

    다음이 줄어드는지 확인:
    - 벽의 좌우 폭 변화
    - 사다리꼴 / shear 느낌
    - 머리 roll 시 벽 비율 변화
    - world swimming / 공간이 휘는 느낌

주의:
    이 케이스는 FOV 자체를 임의 조절하는 실험이 아니다.
    SteamVR가 제공한 projection aspect와 실제 render viewport aspect를
    더 정확히 맞추는 테스트다.
"""
...
def main():
    ...
    # CASE 16 핵심 변경:
    # SteamVR projection frustum aspect를 반드시 사용한다.
    ...
    if args.eye_width > 0 and args.eye_height > 0:
        ...
        if args.use_frustum_aspect:
            ...
            # CASE 16 핵심 변경:
            # 기존 Case 02는 round 후 //8로 다시 내림되어 687.x -> 680이 될 수 있었다.
            # 여기서는 가장 가까운 8픽셀 배수로 반올림한다.
            target_w = EH * aspect
            EW = max(256, int(round(target_w / 8.0)) * 8)
            aspect_mode = "exact projection frustum (nearest 8px)"
        else:
            EW = max(256, int(rec_w * args.eye_scale) // 8 * 8)
            aspect_mode = "SteamVR native"
    print("=" * 72)
    print("CASE 16: EXACT STEAMVR PROJECTION ASPECT")
    if args.use_frustum_aspect and not (args.eye_width > 0 and args.eye_height > 0):
        print(f"[CASE16] target width={target_w:.3f}px -> nearest 8px width={EW}px")
        print(f"[CASE16] projection aspect={aspect:.6f} / render aspect={EW/EH:.6f} "
              f"/ error={abs((EW/EH)-aspect)/aspect*100:.3f}%")
    print("=" * 72)
