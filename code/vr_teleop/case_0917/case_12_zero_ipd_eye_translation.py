"""
CASE 12: ZERO IPD / EYE TRANSLATION OFF
BASE: main_vr_scene_7.py

목적:
    baseline의 좌/우 눈 위치 차이(IPD, eye-to-head translation)가
    실제 Quest/ALVR의 stereo geometry와 어긋나서
    Z-depth가 과장되거나 월드가 기울어 보이는지 확인한다.

이번 케이스에서 바꾼 것:
    - SteamVR에서 받아온 eye-to-head transform의 회전은 그대로 유지
    - 좌/우 눈의 translation(x/y/z)만 0으로 강제
    - 즉, 두 눈의 카메라 중심 위치를 동일하게 만든다
    - projection frustum은 각 눈별 baseline 값을 그대로 사용
...
판정:
    baseline보다 공간의 깊이 왜곡이나 기울기가 크게 줄어들면
    eye translation/IPD 적용이 원인 후보이다.

    반대로 깊이감만 사라지고 월드 휨은 그대로라면,
    현재 문제의 핵심은 IPD보다 projection/head-pose 쪽일 가능성이 높다.

주의:
    이 케이스에서는 두 눈 위치가 같아지므로 정상적인 입체감은 약해지거나 사라질 수 있다.
    "입체감 감소" 자체는 실패가 아니다.
    월드의 안정성, 기울기, Z-depth 왜곡 변화만 비교한다.
...
class EyeRig:
    def __init__(self, vr_system, roll_test=0.0, lock_translation=True):
        ...
        # CASE 12 핵심 변경:
        # eye-to-head의 회전은 보존하고 translation(IPD 포함)만 제거한다.
...
def main():
    ...
    print("=" * 68)
    print("CASE 12: ZERO IPD / EYE TRANSLATION OFF")
    print("baseline 유지 + eye-to-head translation(IPD 포함)만 0으로 강제")
    print("※ 입체감 감소는 정상. 월드 휨/기울기 변화만 비교하세요.")
    print("=" * 68)
